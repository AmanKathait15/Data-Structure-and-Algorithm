...
node* reverse_on_1(node * head)
{
	node *tmp = head , *prev = head; int len=0;

	while(tmp)
	{
		len++;

		if(tmp->info == 1 and len>1)
		{
			if(len==2)
			{
				int t = prev->info; prev->info = tmp->info; tmp->info=t;
			}
			else
			{
				node *temp = tmp->next; tmp->next=NULL;

				prev = reverse(prev); tmp->next=temp; prev=tmp->next;
			}
...
int main()
{
	...
	while(1)
	{
		...
		switch(ch)
		{
			...
			case 'R': head = reverse_on_1(head); break;
