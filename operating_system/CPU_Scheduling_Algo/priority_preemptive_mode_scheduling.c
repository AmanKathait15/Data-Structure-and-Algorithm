...
void sort(int a[][atr], int size)
{
	...
	for(i=0; i<size-1; i++)
	{
		for(j=0; j<size-i-1; j++)
		{
			...
			{
				tmp1=a[j][0]; tmp2=a[j][1]; tmp3=a[j][2]; tmp4=a[j][3]; tmp5=a[j][7];
				a[j][0]=a[j+1][0]; a[j][1]=a[j+1][1]; a[j][2]=a[j+1][2]; a[j][3]=a[j+1][3]; a[j][7]=a[j+1][7];
				a[j+1][0]=tmp1; a[j+1][1]=tmp2; a[j+1][2]=tmp3; a[j+1][3]=tmp4; a[j+1][7]=tmp5;
			}
		}
	}
}
...
void compute(int a[][atr], int size , float *atat , float *awt)
{
	...
	for(i=0; i<size;)
	{
		...
		if(i!=val)
		{
			...
			tmp1=a[val][0]; tmp2=a[val][1]; tmp3=a[val][2]; tmp4=a[val][3]; tmp5=a[val][7];
			a[val][0]=a[i][0]; a[val][1]=a[i][1]; a[val][2]=a[i][2]; a[val][3]=a[i][3]; a[val][7]=a[i][7];
			a[i][0]=tmp1; a[i][1]=tmp2; a[i][2]=tmp3; a[i][3]=tmp4; a[i][7]=tmp5;
		}
...
int main()
{
	...
	printf("\n/* ====== PRIORITY SCHEDULING (preemptive mode) ======= */\n\n");
	printf("\n/* ====================== INPUT =========================== */\n\n");
